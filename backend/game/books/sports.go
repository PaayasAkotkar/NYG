...
func PackSports() Parcel {
	var done = make(chan struct{})
	var run sync.Once
	var parcel Parcel

	App, Conn := resty.New(), resty.NewEventSource().SetURL(FBURLS[0])
	...
	go func() {
		resp, _ := App.R().
			SetHeader("Content-Type", "application/json").SetBody(register).SetResult(&Parcel{}).Post(FBURLS[0])
		if !resp.IsSuccess() {
			return
		}
	}()
